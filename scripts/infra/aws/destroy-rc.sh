...
# Usage: AMPLIFY_GITHUB_TOKEN=<token> bash scripts/infra/aws/destroy-rc.sh <branch-name>
# Example: AMPLIFY_GITHUB_TOKEN=ghp_xxx bash scripts/infra/aws/destroy-rc.sh rc-auth
#
...
if [ -z "${BRANCH}" ]; then
  ...
  echo "Usage: AMPLIFY_GITHUB_TOKEN=<token> bash scripts/infra/aws/destroy-rc.sh <branch-name>" >&2
  exit 1
fi
