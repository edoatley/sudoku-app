...
test('number-first flow — select number then cell', async ({ page }) => {
  ...
  await page.getByTestId('numberpad-normal').getByRole('button', { name: '6', exact: true }).click();
...
test('candidate toggle-off — adding then removing a candidate', async ({ page }) => {
  ...
  // First cycle: add candidate 4 to cell-0-2 (using candidate row switches to candidate mode)
