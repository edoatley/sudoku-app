...
class TestBedrockHandlerE2E:
    @pytest.mark.parametrize(
        "name,path,min_clues,expected_empty_cells,expected_grid",
        _PUZZLES,
        ids=_PUZZLE_IDS,
    )
    def test_fixture_image_exists(
        self, name, path, min_clues, expected_empty_cells, expected_grid
    ):
        assert path.exists(), (
    ...
    def test_handler_returns_valid_9x9_grid(
        self, name, path, min_clues, expected_empty_cells, expected_grid
    ):
        if not path.exists():
            pytest.skip(f"{name}: fixture image not found at {path}")
    ...
    def test_grid_values_are_valid_digits(
        self, name, path, min_clues, expected_empty_cells, expected_grid
    ):
        if not path.exists():
            pytest.skip(f"{name}: fixture image not found at {path}")
    ...
    def test_grid_has_minimum_clues(
        self, name, path, min_clues, expected_empty_cells, expected_grid
    ):
        if not path.exists():
            pytest.skip(f"{name}: fixture image not found at {path}")
    ...
    def test_print_recognized_grid(
        self, name, path, min_clues, expected_empty_cells, expected_grid, capsys
    ):
    ...
    def test_grid_matches_expected(
        self, name, path, min_clues, expected_empty_cells, expected_grid
    ):
        ...
        diff_lines = "\n".join(
            f"  [{r}][{c}]: expected {exp}, got {act}" for r, c, exp, act in wrong
        )
        assert not wrong, (
