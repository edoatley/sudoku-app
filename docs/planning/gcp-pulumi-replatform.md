...
- Phase 2 is gated on the **GCP project-quota check** (§6, R1) — verify *before* writing the PR.
...
| # | Risk | Mitigation |
| --- | --- | --- |
| R1 | **GCP project quota blocks Phase 2.** A personal billing account allows few projects, and a deleted project's ID is unusable for 30 days. | Check Console → IAM & Admin → Quotas (metric `Projects`) **before writing Phase 2**; request an increase early (multi-day turnaround). Use a genuinely new id. If refused, the fallback is rebuilding in the existing project under a new Firestore database and Hosting site — which **forfeits the "100% Pulumi-built" property**. Stated here so it is a decision, not a discovery. |
