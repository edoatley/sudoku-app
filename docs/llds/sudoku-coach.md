...
5. Parsing the structured JSON response `{ "aiMessage": "...", "revealHint": true|false }`,
   server-enforced by Bedrock structured output rather than prompt wording alone
...
| `COACH_BEDROCK_API_MODE` Lambda env var defaults to `converse` on all `rc-*` workspaces, `invoke` on `default` (`infra/main.tf` `local.coach_bedrock_api_mode`) | Same `is_default`/`is_rc` ternary pattern already used for `ai_coach_enabled`/`VITE_AI_COACH` | A per-branch Terraform var override; a workflow_dispatch input | RC is where new coach behaviour is A/B-tested before considering it for production; mirrors the existing convention rather than introducing a new one. No IAM change needed — AWS's Converse API reuses the `bedrock:InvokeModel` permission the Lambda role already has |
