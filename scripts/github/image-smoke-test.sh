#!/usr/bin/env bash
# Image recognition smoke test — POST a base64-encoded puzzle image to /api/v1/ai/image-to-puzzle.
#
...
echo "POST /api/v1/ai/image-to-puzzle → HTTP ${STATUS}"
cat /tmp/import.json
...
if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  {
    ...
    echo "|----------|--------|"
    echo "| POST /api/v1/ai/image-to-puzzle | ✅ ${STATUS} — ${ROWS} rows |"
  } >> "${GITHUB_STEP_SUMMARY}"
fi
