...
# (via the existing delegate-dns.sh). Analogous to gcp-aws-iam-bootstrap.sh spanning both clouds.
#
...
exec "${HERE}/delegate-dns.sh" --subdomain "${SUBDOMAIN}" "${NS[0]}" "${NS[1]}" "${NS[2]}" "${NS[3]}"
