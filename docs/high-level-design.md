...
| DNS / TLS | Route53 + ACM | Cloud DNS (`gcp.edoatley.co.uk`, delegated) + Google-managed TLS |
| Cost guardrail | AWS Budgets + Cost Anomaly | Cloud Billing Budget + Pub/Sub |
| State backend / image registry | Terraform, S3 backend + ECR | Pulumi (Python/uv), GCS state + Cloud KMS secrets + Artifact Registry |
| CI identity federation | GitHub OIDC → IAM role | Workload Identity Federation → deploy service account |
| AI inference | Amazon Bedrock (Claude Haiku) | Vertex AI (Gemini) — coach and image recognition; no cross-cloud dependency |
...
- **Two-stack privilege split.** On GCP, everything is IaC (Pulumi), but the deploying identity is split in two: a `bootstrap` stack run locally with human credentials owns the project, billing, KMS, service accounts, IAM role bindings, Workload Identity Federation and Artifact Registry; an `app` stack run by CI as the WIF-federated deploy service account owns only application resources, and holds no IAM-admin, WIF-admin, or billing-account permission. Two steps stay manual because no API exists for them: the project/billing/state prelude (Pulumi cannot bootstrap its own state backend) and the OAuth consent screen + OAuth client. Cloud Run reaches Firestore over Google's managed public API; private VPC egress is an unbuilt option.
...
- **AI provider.** GCP inference is Vertex AI (Gemini), authenticated by each runtime service account via ADC. The **coach** moved first behind the `CoachAiClient` port (`coach.ai.provider=vertex`, PR #212); **image recognition** follows behind an `IMAGE_AI_PROVIDER` switch of the same shape, gated on a fixture-based accuracy comparison against the incumbent Bedrock Claude Haiku path. Once both have moved, the interim cross-cloud AWS access key (`CP-GCP-085`) and Secret Manager itself are removed from the project — leaving GCP with **zero long-lived credentials**.
- **Functional at parity (as an alternate deployment target).** The GCP facet runs end-to-end: Firestore persistence (games, players, leaderboard, admin, coach rate-limit), in-app Firebase-token auth with Google-`sub` identity, the image-recognition Cloud Run service, and the Vertex AI coach. It is now being **re-platformed from Terraform to Pulumi** (Python + `uv`) in a clean-room GCP project, which also removes the last AWS runtime dependencies and introduces a single `DEPLOY_TARGET` repository variable selecting which cloud a merge to `main` deploys — so deployment mechanics stop differing between the two. Remaining deferred items: budget hard-cap, private VPC egress, GCP admin authorization, and the AWS Cognito→Google-`sub` re-key. See `docs/llds/cloud-platform-gcp.md` (design), `docs/planning/gcp-pulumi-replatform.md` (migration), `docs/aws-vs-gcp-comparison.md` (architecture), `docs/aws-vs-gcp-deployment.md` (pipelines + how to choose a target), and the `gcp-manual-setup.md` runbook.
...
- Prefer a **two-stack privilege split** on GCP over either fully-manual identity or a single all-powerful deploy identity. A human-credentialed `bootstrap` stack owns the project, billing, KMS, service accounts, IAM and federation; a CI-credentialed `app` stack owns only application resources, cannot grant itself privilege, and holds no billing-account permission. The identity layer remains the learning surface — it is now *declared and reviewable* rather than *hand-typed and undocumented*. *(Revised 2026-09-11, superseding "prefer manual, least-privilege identity on GCP over Terraform-managed IAM": the manual tenet produced six bash scripts that were the least-tested, least-reviewed and most drift-prone part of the system, while the privilege concern it protected is better served by splitting the deploying identity in two than by removing it from automation. Net change for the CI identity: +1 role, −all Secret Manager roles, no billing reach.)*
- Prefer **no cross-cloud runtime credentials** — each cloud's deployment authenticates only with that cloud's native identity. (Retires the interim AWS access key held in GCP Secret Manager.)
...
| **Cloud Platform**                 | All cloud infrastructure at parity across two providers — AWS (Lambda, API GW, DynamoDB, Cognito, Amplify, Route53, IAM) via Terraform, and GCP (Cloud Run, Firestore, Identity Platform, Firebase Hosting, Cloud DNS) via Pulumi | `infra/aws/*.tf` (Terraform), `infra/gcp/**` (Pulumi, Python) |
...
| AI provider on GCP       | Image recognition still calls Bedrock cross-cloud via an AWS credential in Secret Manager | The coach moved to Vertex (`CP-GCP-090`, PR #212). Image recognition follows behind `IMAGE_AI_PROVIDER` (`CP-GCP-089`), gated on a fixture accuracy comparison; only then can the AWS key (`CP-GCP-085`) be retired |
| GCP project quota        | A clean-room rebuild needs a second project alongside the live one | Personal billing accounts cap project count, and a deleted id is unusable for 30 days. Verify headroom before the bootstrap phase — it is a hard blocker |
| OAuth client creation    | No GCP API exists to create OAuth 2.0 client IDs        | Irreducibly manual on both clouds. The current client is shared between Cognito and Identity Platform and lives in the old project, so retiring that project needs a second client for AWS first |
