...
- **AI provider.** The GCP backend calls Amazon Bedrock cross-cloud at first (no app-code change; an AWS credential lives in Secret Manager). The GCP-native end state is Vertex AI (Gemini), authenticated by the runtime service account — **now an active gap** (`CP-GCP-090`): the **coach** moves first, behind a new `CoachAiClient` port (`coach.ai.provider`), which removes the coach's dependence on the cross-cloud AWS key. Image-recognition inference stays cross-cloud until it too migrates, so the interim AWS key (`CP-GCP-085`) is retired only after both.
- **Functional at parity (as an alternate deployment target).** The GCP facet began as infrastructure scaffolding (Terraform stands up Cloud Run, Firestore, Firebase Hosting, DNS, a budget) and has since been made to run end-to-end: the app-layer adapters all landed — Firestore persistence (games, players, leaderboard, admin, coach rate-limit), in-app Firebase-token auth with Google-`sub` identity, the image-recognition Cloud Run service, and cross-cloud Bedrock. A deployed GCP env is reachable at a custom DNS host (`sudoku-gcp.edoatley.co.uk`) with an automated post-deploy smoke. What differs from AWS is now **deployment mechanics** (GCP prod is a manual `workflow_dispatch`; AWS prod is push-to-`main`) and a short list of deferred items (Vertex AI, budget hard-cap, private VPC, the AWS Cognito→Google-`sub` re-key). See `docs/aws-vs-gcp-comparison.md` (architecture), `docs/aws-vs-gcp-deployment.md` (pipelines + how to choose a target), and the `gcp-manual-setup.md` runbook.
...
| AI provider on GCP       | GCP backend calls Bedrock cross-cloud via an AWS credential in Secret Manager | Vertex AI (Gemini) migration is an **active gap** (`CP-GCP-090`): coach first, behind the `CoachAiClient` port; image-rec still cross-cloud |
