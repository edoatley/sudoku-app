...
- [x] **UM-GCP-001**: On GCP the system shall validate the caller's JWT in-app (there is no edge authorizer) and reject any request to a non-public route that lacks a valid token.
- [x] **UM-GCP-002**: On GCP the system shall validate tokens against the Identity Platform issuer https://securetoken.google.com/<project_id> and audience <project_id> using an explicitly configured JWKS, with OIDC discovery disabled.
...
- [x] **UM-GCP-005**: On GCP (Firebase tokens) the system shall reject any authenticated request whose email claim is absent, not on app.allowed.emails, or whose email_verified claim is not true. The email_verified requirement is Firebase-specific — Firebase reports it truthfully (true for verified Google logins) and in-app validation is the sole gate. On AWS (Cognito tokens) the allowlist match alone applies: Cognito sets email_verified=false for every external-provider (Google-federated) user even though the email came from Google, and the request is additionally gated by the API Gateway Cognito authorizer with self-signup disabled, so enforcing email_verified would lock out legitimate users with no security gain.
- [x] **UM-GCP-006**: On GCP the system shall answer CORS preflight OPTIONS requests before authentication, so a cross-origin preflight (which carries no Authorization header) is not rejected.
- [x] **UM-GCP-007**: Where the configuration property sudoku.persistence=firestore (resolved at runtime via a CDI producer), the system shall persist player profiles in a Firestore players collection keyed by the canonical userId, with lazy-creation and PATCH-update behaviour unchanged.
...
- [x] **UM-GCP-010**: The system shall serve the admin data browser (/admin/data/games, /admin/data/players) through an AdminDataRepository port, so the resource no longer injects a cloud SDK directly; where sudoku.persistence=firestore (resolved at runtime via a CDI producer) it shall read the Firestore games and players collections, otherwise it shall scan the DynamoDB tables — with identical response shapes on both clouds. (Data access only; administrator authorization on GCP remains UM-GCP-008.)
