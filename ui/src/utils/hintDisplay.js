...
export function formatHintText(text) {
  ...
  result = result.replace(/\(([0-8]),\s*([0-8])\)/g, (_, r, c) =>
    `(${Number(r) + 1}, ${Number(c) + 1})`
  );
  ...
  result = result.replace(
    ...
    (_, unit, a, b, c) =>
      `${unit} ${Number(a) + 1}, ${Number(b) + 1} and ${Number(c) + 1}`
  );
  ...
  result = result.replace(/\b([Rr]ow|[Cc]olumn) ([0-8])\b/g, (_, unit, n) =>
    `${unit} ${Number(n) + 1}`
  );
